...
(function($) {
    $.fn.formQuiz = function(options) {
        ...
        steps.each(function(i) {
            $(this).wrap('<div data-step="' + i + '"></div>');
            $(this).find('[data-quiz-btn="next"]').attr('disabled', true);

            if (i == 0) options.progress(i, count - 1);
            else element.find('[data-step="' + i + '"]').hide();
        });

        element.go = function(i) {
            element.find('[data-step]').hide();
            element.find('[data-step="' + i + '"]').show();
            options.progress(i, count - 1);
        }

        element.find(':input').bind('change keyup', function(e) {
            var currentStep = element.find('[data-step]:visible'),
                nextButton = currentStep.find('[data-quiz-btn="next"]');

            if (options.validateBeforeNext(element, currentStep) === true) {
        ...
        element.find('[data-quiz-btn="next"]').bind("click", function(e) {
            e.preventDefault();
            var currentStep = element.find('[data-step]:visible'),
                targetStep = currentStep.data('step') + 1;

            if (options.validateBeforeNext(element, currentStep) === true) {
                element.find('[data-step]').hide();
                element.find('[data-step="' + targetStep + '"]').show();
                options.progress(targetStep, count - 1);
            }

            return false;
        });

        element.find('[data-quiz-btn="prev"]').bind("click", function(e) {
            e.preventDefault();
            var currentStep = element.find('[data-step]:visible'),
                targetStep = currentStep.data('step') - 1;
        ...
        element.find('[data-quiz-btn="skip"]').bind("click", function(e) {
            e.preventDefault();
            var currentStep = element.find('[data-step]:visible'),
                targetStep = currentStep.data('step') + 1;
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    $("[data-scroll-to]").on('click', function(e) {
        ...
        if ($(target).length > 0) {
            ...
            console.log('scroll-to: [' + target + '] not found!');
        }
    });
    ...
    // more
    $('.show-logolist').on('click', function() {
        $('.logolist-hidden').removeClass('logolist-hidden');
        $(this).hide();
    });

    $('.show-portfoliolist').on('click', function() {
        $('.portfoliolist-hidden').removeClass('portfoliolist-hidden');
        $(this).hide();
    });
   ...
   /*  $('[name="phone"]').inputmask('+7 (999) 999-99-99'); */
    ...
    $('[data-range]').on('input', function() {
        var target = $(this).data('range-for'),
            value = $(this).val();

        $('[data-number="' + target + '"]').val(value + ' m');
    });
    ...
    $('[data-number]').on('keyup', function() {
        ...
        if (value == '') $(this).val(value);
            ...
            $(this).val(value + ' m');
            if (this.selectionStart) this.setSelectionRange(value.length, value.length);
            $('[data-range-for="' + target + '"]').val(value);
        }
    });
    ...
    $('.form-js').submit(function() {
        ...
        $.ajax({
            ...
            .always(function() {
                th.trigger('reset');
                th.data().isSubmitted = false;
            });

        th.data().isSubmitted = true;
    ...
    var quiz = $('#quiz').formQuiz({
        validateBeforeNext: function(form, step) {
            ...
            var validator = form.validate({ errorElement: 'em' });
            $(':input', step).each(function(index, el) {
        ...
        progress: function(i, count) {
            $('[data-progress="line"]').width((i / count * 100) + '%');
            $('[data-progress="text"]').text(Math.round(i / count * 100) + '%');
        }
    });

    $('.quiz-prepare').on('click', 'button', function() {
        var val = $(this).text();
        if (val == 'Выберите из списка') {
            quiz.go(0);
        } else {
    ...
    $('[href^="mdlt-"], [data-mdlt^="mdlt-"]').click(function() {
        var modal = $(this).attr('href') || $(this).data('mdlt');
        var title = $(this).data('mdlt-title') || false;
        modalName = modal.slice(5);

        if (modalName == 'quiz') {
            $('[data-prepare]').val('Не выбрано');
            quiz.go(0);
        }

        modalityOpen(modalName, title);

        return false;
    });

    $('.modality').click(function(event) {
        if ($(event.target).is('.modality')) {
            var modal = $(event.target).data('mdlt-name');
            modalityClose(modal);
        }
    });

    $('[data-mdlt-close]').click(function(event) {
        var modal = $(this).closest('.modality--open').data('mdlt-name');
        modalityClose(modal);
    });

    function modalityOpen(name, title) {
        var target = '[data-mdlt-name="' + name + '"]';

        if (title) $(target).find('.modality__title').text(title);
        ...
        if ($(target).length > 0) {
            ...
            console.log('Modality: [' + name + '] not found!');
        }
    }

    function modalityClose(name) {
        if (name !== undefined) {
            var target = '[data-mdlt-name="' + name + '"]';

            if ($(target).length > 0) {
                $(target).removeClass('modality--open');
            } else {
                console.log('Modality: [' + name + '] not found!');
            }

            if ($('.modality--open').length == 0) {
    ...
    function maskValidator() {
        var th = $(this),
            phoneValue = $(th).find('[name="phone"]').val(),
            phoneRegExp = /^\+7\s\([0-9]{3}\)\s[0-9]{3}-[0-9]{2}-[0-9]{2}$/i;

        if (!phoneRegExp.test(phoneValue)) {
            $(th).find('[name="phone"]').focus();
            return false;
        }
        return true;
    }
